...
CollisionManager.prototype.beamCollision = function(beam) {
	...
	for (var i = this.state.enemyManager.enemies.members.length - 1; i >= 0; i--) {
		// if( this.state.enemyManager.enemies.members[i].aplha > 0 ){
...
CollisionManager.prototype.releaseGhost = function( ghost ) {
	...
	if(ghost.releaseTime == null && ghost.hit){
		console.log( "Release Ghost" );
		ghost.releaseTime = this.state.game.time.now() + 2000; 
	} else if( ghost.releaseTime < this.state.game.time.now() && ghost.hit){
		ghost.hit = false;
		ghost.releaseTime = null;
	}
	// ghost.hit = false;
